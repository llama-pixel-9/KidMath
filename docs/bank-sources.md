...
assessment: division `remainders` [7,10] (184), placeValue `rounding`
[4,6]+[7,10] (324), and multi-digit enrichment at [8,10] for multiplication
(162), addition (108) and subtraction (108). Generated by
`scripts/itemGen/authorGrade4.js` through the shared `bankAssembler.js`;
design notes in `docs/grade4-bank-design.md`.

### Structural inspirations (no verbatim text)

- CCSS 4.OA.3 (remainder interpretation), 4.NBT.3 (rounding), 4.NBT.4-6
  (multi-digit arithmetic), 3.NBT.1-2.
- EngageNY G4 M1/M3, G3 M1/M2 — structure and sequencing only.
- OA Progressions: the leftover vs round-up remainder distinction.
