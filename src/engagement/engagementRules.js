/**
 * The engagement RULES — pure. engagementStore.js wraps these with the
 * per-kid localStorage IO. Split out so the native engine bundle can share
 * them: iOS applies the same session-end transition (streak, daily goal,
 * badges, egg warmth) and the same sticker spend, so the two apps can never
 * disagree about what a kid earned.
 */
...
export function applySpend(state, sticker) {
  // A blob written by a build that predates stickers has no array yet.
  const owned = Array.isArray(state.stickers) ? state.stickers : [];
  if (!sticker || owned.includes(sticker.id)) return null;
  if (starBalance(state) < sticker.cost) return null;
  return {
    ...state,
    spentStars: (state.spentStars ?? 0) + sticker.cost,
    stickers: [...owned, sticker.id],
  };
}
...
/**
 * Structures whose wording pulls the wrong way ("3 fewer… so ADD"). A
 * first-try win on one of these feeds the Word Detective badge.
 */
export const LANGUAGE_TRAP_STRUCTURES = new Set(["compareBiggerFewer", "compareSmallerMore"]);

export function isLanguageTrapWin(question, isRetry) {
  return !isRetry && LANGUAGE_TRAP_STRUCTURES.has(question?.metadata?.structureType);
}
