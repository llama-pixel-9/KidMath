...
export function pictographConceptual() {
  ...
  // The chart is SHOWN (key in the legend); the claim ignores it.
  const keyIgnoredPhrs = [
    (nm, sym, label) => `${nm} says this chart shows ${sym} ${label}. Is ${nm} right?`,
    (nm, sym, label) => `Reading this chart, ${nm} announces ${sym} ${label}. Is that right?`,
  ];
    ...
    for (let vi = 0; vi < 18; vi += 1) {
      ...
      items.push(
        item("pictograph", "conceptual", structureType, band, {
          answer: "No",
          choices: ["Yes", "No"],
          display: { ...pictoDisplay([{ label, symbols: sym }], key), data: { kind: "keyIgnored", key, sym }, promptText: keyIgnoredPhrs[Math.floor(vi / 9)](nameAt(vi * 3 + 1 + off), sym, label, key), truth: false },
        })
      );
    }
  };
  ...
  const countJudgePhrs = [
    (nm, sym, label, said) => `${nm} counts the ${label} on this chart and says ${said}. Is ${nm} right?`,
    (nm, sym, label, said) => `${nm} reads this chart and reports ${said} for ${label}. Is that right?`,
  ];
  for (let vi = 0; vi < 22; vi += 1) {
    ...
    items.push(
      item("pictograph", "conceptual", "countJudgeTeen", "band1", {
        answer: ok ? "Yes" : "No",
        choices: ["Yes", "No"],
        display: { ...pictoDisplay([{ label, symbols: sym }], 1), data: { kind: "pictoSaid", sym, key: 1, said }, promptText: countJudgePhrs[Math.floor(vi / 11)](nameAt(vi * 3 + 2), sym, label, said), truth: ok },
      })
    );
  }
  const rowMorePhrs = [
  ...
  const halfPhrs = [
    (nm, key) => `A row on this chart ends in half a picture. ${nm} says the half counts as ${key / 2}. Is ${nm} right?`,
    (nm, key) => `${nm} counts the half picture at the end of this chart's row as ${key / 2}. Is that right?`,
  ];
    ...
    for (let vi = 0; vi < n; vi += 1) {
      items.push(
        item("pictograph", "conceptual", structureType, band, {
          answer: "Yes",
          choices: ["Yes", "No"],
          display: { ...pictoDisplay([{ label: SETS[vi % SETS.length].labels[vi % 4], symbols: 2, half: true }], key), data: { kind: "halfSymbol", key }, promptText: halfPhrs[Math.floor(vi / (n / 2))](nameAt(vi * 3 + 5 + off), key), truth: true },
        })
      );
    }
  };
