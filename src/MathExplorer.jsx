...
function NumberBond({ onSubmit, feedback, theme, lowMotionMode, lowEndDevice, whole, part }) {
  const [entry, setEntry] = useState("");
  const locked = feedback === "correct" || feedback === "wrong";
  const pressDigit = (d) => {
...
function BarModel({ onSubmit, feedback, theme, lowMotionMode, lowEndDevice, spec }) {
  const [entry, setEntry] = useState("");
  const locked = feedback === "correct" || feedback === "wrong";
  const pressDigit = (d) => {
...
function clockHand(cx, cy, len, angleDeg) {
  const rad = ((angleDeg - 90) * Math.PI) / 180;
  return { x2: cx + len * Math.cos(rad), y2: cy + len * Math.sin(rad) };
}

function AnalogClock({ onSubmit, feedback, theme, lowMotionMode, lowEndDevice, hour, minute }) {
  const [entry, setEntry] = useState("");
  const locked = feedback === "correct" || feedback === "wrong";
  const pressDigit = (d) => {
...
function FractionSet({ onSubmit, feedback, theme, lowMotionMode, lowEndDevice, set }) {
  const [entry, setEntry] = useState("");
  const locked = feedback === "correct" || feedback === "wrong";
  const pressDigit = (d) => {
    if (!locked) setEntry((e) => (e.length < 5 ? e + d : e));
...
function PlaceValueDiscs({ onSubmit, feedback, theme, lowMotionMode, lowEndDevice, cols }) {
  const [entry, setEntry] = useState("");
  const locked = feedback === "correct" || feedback === "wrong";
  const pressDigit = (d) => {
...
function SymbolSelect({ onSubmit, feedback, revealAnswer, shakenChoice, theme, lowMotionMode, lowEndDevice }) {
  const locked = feedback === "correct" || feedback === "wrong";
  return (
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 gap-6">
        ...
        {answerType === "numberPad" || answerType === "fillBlank" || answerType === "decimal" ? (
          ...
          <AnalogClock
            ...
            lowMotionMode={lowMotionMode}
            lowEndDevice={lowEndDevice}
            hour={currentQ.display?.hour}
            minute={currentQ.display?.minute}
          />
          ...
          <NumberBond
            ...
            lowMotionMode={lowMotionMode}
            lowEndDevice={lowEndDevice}
            whole={currentQ.display?.whole}
            part={currentQ.display?.part}
          />
