...
| 11 | **Meadow art.** `npm run ios:art` (`scripts/syncMeadowArt.mjs`) copies `public/meadow/**.webp` into `ios/KidMath/Resources/meadow/` as flat loose files (Xcode's asset compiler rejects WebP; ImageIO decodes it at runtime) and generates `MeadowArtManifest.swift`. `MeadowArt` mirrors artAssets.js (bird/variant, prop, feather, zone, egg stage; cached decode). `BirdSpriteView` shows the art (sketch fallback), `PerchedBirdView` passes species + cling/fly variants, `ZoneBackdropView` uses the zone art + props from the shared `SCENERY` (moved to `meadow/scenery.js`, on the bridge as `meadowScenery`), `EggSpriteView` uses the crack stages, badges use feather art (`BadgeGlyph`). `MeadowArtTests` + rendered snapshot. | med | M | ☑ PR ios/parity-10 |
...
parity-9 (☑ #5) → parity-10 (☑ #11) → #13 #15 #16 #17 + the cosmetic tail of #8.
